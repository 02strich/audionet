...
class OggPage(object):
	...
	def pack(self):
		header = struct.pack("<4sBBqLL", 'OggS', self.version, self.headerType, self.granulePosition, self.streamID, self.sequenceNumber)
		def _(s):
			return chr(len(s))
		body = ''.join( [ chr(len(self.segments)) ] + map(_, self.segments) + self.segments )
		return ''.join([ header, struct.pack("<L", checksum(header, body)), body ])
	def __repr__(self):
...
class iobuffer(object):
	...
	def write(self, data):
		self.data = ''.join([ self.data[self.offset:], data ])
		self.offset = 0
		self.length += len(data)
	def __len__(self):
		return self.length
	...
	def unread(self, data):
		self.data = ''.join([ data, self.data[self.offset:] ])
		self.offset = 0
		self.length += len(data)
...
class VorbisStreamReader(OggStreamReader):
	...
	def read_comments(self, packet):
		...
		comments.vendor = s.read(length).decode('utf-8')
		comments.comments = []
		ncomments, = s.unpack("<L")
		for i in xrange(ncomments):
			length, = s.unpack("<L")
			key, value = s.read(length).split('=', 1)
			comments.comments.append([key.upper(), value.decode('utf-8')])
		assert s.read(1) == '\x01'
		assert len(s) == 0
...
if __name__ == "__main__":
	...
	stream.process(file('test.ogg', 'rb').read())
	print 'size            :', info.payload
	print 'overhead        :', info.overhead
	print 'pages           :', info.pageCount
	print 'audio packets   :', info.audioPackets
	print 'audio length    :', info.audioPacketLength 
	print 'vendor          :', info.comments.vendor
	print 'comment entries :', len(info.comments.comments)
	for key, value in info.comments.comments:
		print '- %s: %s'%(key, value)
	print 'sample rate     :', info.identification.sampleRate
	print 'audio channels  :', info.identification.audioChannels
	print 'nominal bitrate :', info.identification.nominalBitrate
	print "first frame     : %fs" % info.start
	import datetime
	print "duration        :", datetime.timedelta(seconds=info.stop-info.start)
